...
function intersectIntervals(a,b) {
  ...
  for (const ia of a) {
    for (const ib of b) {
      const s = maxHM(ia.start, ib.start);
      const e = minHM(ia.end, ib.end);
...
function subtractIntervals(freeMins, busyMins) {
  ...
  for (const b of busyMins) {
    ...
    for (const f of res) {
      ...
      if (b.start > f.start) {
        next.push(iv(f.start, Math.max(f.start, b.start)));
      }

      if (b.end < f.end) {
        next.push(iv(Math.min(f.end, b.end), f.end));
      }
    }
...
function canEmployeeDo(service, employee) {
  ...
  const sids = Array.isArray(employee.serviceId)
    ? employee.serviceId
    : Array.isArray(employee.serviceIds)
      ? employee.serviceIds
      : [];
...
export default function BookingTime() {
  ...
  const [selectedIds, setSelectedIds] = useState(() => {
    ...
    return cart
      .filter(s => canEmployeeDo(s, chosenEmployee))
      .map(s => s.serviceId);
  });
  ...
  const totalAmountRsd = selectedServices.reduce(
    (a,b) => a + (Number(b.priceRsd) || 0),
    0
  );
  ...
  useEffect(() => {
    ...
    const usernames =
      chosenEmployeeId === "firstFree"
        ? targetUsernames.split("|").filter(Boolean)
        : [targetUsernames];
  ...
  }, [selectedDay, targetUsernames, chosenEmployeeId, today0, maxDate]);
  ...
  const { slots, anyWork } = useMemo(() => {
    ...
    if (chosenEmployeeId !== "firstFree") {
      ...
      for (const e of employees) {
        ...
        const ss = freeForEmp(e.username, day).map(s => ({
          ...s,
          employeeId: e.username
        }));
  ...
  function canDoEmployee(s, employee) {
    return canEmployeeDo(s, employee);
  }
  ...
  function ensureValidSelectionForEmployee(cb) {
    if (chosenEmployeeId === "firstFree" || !chosenEmployee) {
      cb();
      return;
    }
    ...
    if (doable.length === 0) {
      setMismatch({
        ...
        notDoable,
        proceedCb: null,
        backCb: () => nav("/booking/employee", {
          state: { reason: "employee-does-not-do-selected-services" }
        })
      });
    ...
    setMismatch({
      ...
      backCb: () => nav("/booking/employee", {
        state: { reason: "pick-employee-for-remaining" }
      })
    });
  }
  ...
  function pickSlot(opt) {
    ...
    ensureValidSelectionForEmployee(() => {
      ...
      if (chosenEmployee) {
        pushToast(
          `Radnik: ${
            `${chosenEmployee.firstName || ""} ${chosenEmployee.lastName || ""}`.trim() ||
  ...
  async function confirmBooking() {
    ...
    try {
      ...
      for (let i = 0; i < groups.length; i++) {
        const g = groups[i];
        const gDuration = g.services.reduce(
          (a,b) => a + (Number(b.durationMin) || 0),
          0
        );

        const gAmount = g.services.reduce(
          (a,b) => a + (Number(b.priceRsd) || 0),
          0
        );
      ...
      const appointmentEmployee = employees.find(
        e => e.username === (confirmData.employeeId || "")
      );

      const employeeName =
        appointmentEmployee
          ? `${appointmentEmployee.firstName || ""} ${appointmentEmployee.lastName || ""}`.trim() ||
            appointmentEmployee.username
          : "";

      setCalendarEvent({
        title: `Termin - ${selectedServices.map(s => s.name).join(", ")}`,
        start: new Date(confirmData.start),
        end: new Date(rollingStart),
        location: "Makenzijeva 26, Beograd",
        description:
          `Zakazan termin u salonu Impresivna i svoja.\n` +
          `Usluge: ${selectedServices.map(s => s.name).join(", ")}\n` +
          `Zaposleni: ${employeeName || "nije navedeno"}\n` +
          `Ukupno: ${totalAmountRsd.toLocaleString("sr-RS")} RSD`,
      });

      try {
        const emp = appointmentEmployee;
      ...
      if (remaining.length > 0) {
        ...
        setTimeout(() => {
          nav("/booking/employee", {
            state: { info: `Ostalo za zakazivanje: ${left} usluga` }
          });
        }, 400);
      } else {
        pushToast("Termin je uspešno zakazan. Možete ga dodati u kalendar.");
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="wrap">
      ...
      <div className="sheet">
        ...
        <div className="title">
          {nice(anchor, { month: "long", year: "numeric" })}
        </div>
        ...
        <div className="cal">
          ...
          <div className="grid">
            ...
            {(() => {
              const firstDow =
                (new Date(anchor.getFullYear(), anchor.getMonth(), 1).getDay() + 6) % 7;
                ...
                const title = disabled
                  ? (
                    isPastDay
                      ...
                      : (
                        isTooFar
                          ? "Zakazivanje moguće samo za nedelju unapred"
                          : "Salon ne radi"
                      )
                  )
                  : "";
        ...
        <div className="res">
          <div className="helpers">
            <button className="pill" onClick={selectAllForEmployee}>
              Uključi sve koje ova radnica radi
            </button>
                  ...
                  <div style={{ fontSize: 12, opacity: .7 }}>
                    ...
                    {chosenEmployeeId !== "firstFree" && chosenEmployee && (
                      canEmployeeDo(it, chosenEmployee)
                        ? (
                          <span style={{ marginLeft: 8, color: "#208A3C" }}>
                            • radi ova radnica
                          </span>
                        )
                        : (
                          <span style={{ marginLeft: 8, color: "#B02A37" }}>
                            • ova radnica ne radi
                          </span>
                        )
                    )}
          ...
          <div className="sum">
            <span>
              Za zakazivanje sada: {selectedServices.length} / {cart.length} usl.
            </span>

            <span>
              {totalAmountRsd.toLocaleString("sr-RS")} RSD • {totalDurationMin} min
            </span>
          </div>

          {remainingServices.length > 0 && (
            <div style={{ marginTop: 6, fontSize: 13, opacity: .75 }}>
              Ostaje za kasnije: {remainingServices.length} usluga
              {" "}
              (posle potvrde bićeš vraćen na izbor radnika za preostalo).
            </div>
          )}
        </div>
          ...
          >
            ...
            >
              ...
              <div className="cs-row">
                <div>Datum</div>
                <div>{niceDate(confirmData.start)}</div>
              </div>

              <div className="cs-row">
                <div>Vreme</div>
                <div>{hhmm(confirmData.start)} – {hhmm(confirmData.end)}</div>
              </div>
              ...
              <div className="cs-row">
                ...
                <div>
                  {(() => {
                    ...
                    return e
                      ? `${e.firstName || ""} ${e.lastName || ""}`.trim() || id
                      : id;
                  })()}
                </div>
              </div>
              ...
              {selectedServices.map(s => (
                <div key={s.serviceId} className="cs-row">
                  <div>{s.name}</div>
                  <div>
                    {Number(s.priceRsd || 0).toLocaleString("sr-RS")} • {s.durationMin}m
                  </div>
                </div>
              ))}
              ...
              <div className="cs-row">
                ...
                <div style={{ fontWeight: 900 }}>
                  {totalAmountRsd.toLocaleString("sr-RS")} RSD • {totalDurationMin}m
                </div>
              </div>
        ...
        {mismatch.open && (
          <div
            ...
            onClick={() => setMismatch(m => ({ ...m, open: false }))}
          >
            <div
              className="mm-card"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="mm-head">
                ...
                <div className="mm-title">
                  Ne poklapaju se usluge
                </div>
              </div>

              <div className="mm-section">
                <div className="mm-sub">
                  Ova radnica <b>NE radi</b>:
                </div>

                <ul className="mm-list">
                  {mismatch.notDoable.map(s => (
                    <li key={s.serviceId}>
                      • {s.name}
                    </li>
                  ))}
