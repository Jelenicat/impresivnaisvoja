...
export default async function handler(req, res) {
  try {
    ...
    for (const doc of snap.docs) {
      ...
      if (!appt.clientId || appt.remind2hSent) continue;
      ...
      await doc.ref.update({ remind2hSent: true, remind2hAt: admin.firestore.FieldValue.serverTimestamp() });
      count++;
    }

    res.status(200).send(`OK 2h reminders sent: ${count}`);
  } catch (err) {
    console.error(err);
    res.status(500).send("Error sending 2h reminders");
  }
}
